...
class TestScanCommand:
    ...
    def test_scan_json(self, demo_dir, tmp_path):
        out = str(tmp_path / "report.json")
        result = runner.invoke(main, ["scan", f"{demo_dir}/registry-manifest.yaml", "--format", "json", "-o", out])
        assert result.exit_code == 0
...
class TestCleanupCommand:
    ...
    def test_cleanup_with_policy(self, demo_dir):
        result = runner.invoke(main, ["cleanup", f"{demo_dir}/registry-manifest.yaml", "-p", f"{demo_dir}/cleanup-policy.yaml"])
        assert result.exit_code == 0

    def test_cleanup_json(self, demo_dir):
        result = runner.invoke(main, ["cleanup", f"{demo_dir}/registry-manifest.yaml", "--format", "json"])
        assert result.exit_code == 0
...
class TestAuditCommand:
    ...
    def test_audit_json(self, demo_dir):
        result = runner.invoke(main, ["audit", f"{demo_dir}/registry-manifest.yaml", "--format", "json"])
        assert result.exit_code == 0
        assert "passed" in result.output

    def test_audit_fail_on(self, demo_dir):
        result = runner.invoke(main, ["audit", f"{demo_dir}/registry-manifest.yaml", "--fail-on", "critical"])
